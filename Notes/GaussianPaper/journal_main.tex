...
The aim of this work is to contribute to such an understanding by concentrating on a simple, exactly solvable fermionic model. This allows us to derive analytical formulae for the cooling rates, the final energies and fidelities, as well as to investigate the limitations posed by the presence of noise. With that knowledge, we devise some refinements that improve the cooling algorithm and also develop re-optimization techniques that improve the performance, especially in noisy environments. We also compare the efficiency between cooling and DSP and find parameter regimes in this solvable model where cooling gives lower relative energies. While all these results are specific for the simple model we analyze, we believe that some of the techniques developed here may become relevant in other, more general, situations.
...
\begin{equation}
    H_{S} = \sum_{k=0}^{N/2} \eta_k h_k,
    \label{eq:Hs_grouped}
\end{equation}
where the mode Hamiltonians $h_k$ can be read off from~\cref{eq:Hs}: $h_k=\hat{a}^\dagger_k \hat{a}_k-\hat{a}_{-k}\hat{a}_{-k}^\dagger$ for $k=1,\ldots,N/2-1$ and $h_k=\hat{a}_k^\dagger \hat{a}_k-\hat{a}_k \hat{a}_k^\dagger$ for $k=0,N/2$.  The grouped coefficient is
\begin{equation}
    \eta_k =
    \begin{cases}
        \epsilon_k, & k=1,\ldots,N/2-1,\\[2mm]
        \epsilon_k/2, & k=0,N/2.
    \end{cases}
    \label{eq:grouped_mode_weight}
\end{equation}
The factor $1/2$ for the special modes is inherited from the full-grid formula
\cref{eq:Hs}: the generic block contains the two modes $k$ and $-k$, whereas
the modes $0$ and $N/2$ are unpaired.
...
\begin{equation}
    \frac{1}{N}\sum_{k=0}^{N/2} \left|\tr\left(\eta_k h_k \left[\mathcal{E}_k^n(\rho_k)- \sigma_k\right]\right)\right|\le \frac{\sum_{k=0}^{N/2} \eta_k e^{-\alpha_k n}}{N} \le C e^{- \alpha n},
\end{equation}
where $C=O(1)$ is independent of $N$. Thus, if $\alpha>0$ and we aim for an energy density error of $\epsilon$, we need $n^c=\alpha^{-1}\log(C/\epsilon)$ cycles, independent of $N$.
...
\begin{equation}
    E =\sum_{k=0}^{N/2} E_k = \sum_{k=0}^{N/2} \eta_k \tr( h_k \sigma_k).
\end{equation}
For modes $(k,-k)$ (with $k=1,\ldots,N/2-1$), the ground state energy contribution is $-\epsilon_k$.  For $k=0,N/2$ it is instead $-\epsilon_k/2$.  Equivalently, with the grouped coefficient in \cref{eq:grouped_mode_weight}, one may write the block relative energy uniformly as $e_k=(E_k+\eta_k)/\eta_k$. The total relative energy is given by \cref{eq:relative_energy}, with $E_{GS}$ given in \cref{eq:EnergyGS}.
...
Next, let us consider the effect of increasing the number of bath frequencies $R$ for fixed $g$ and $\kappa$ (i.e., comparing different line styles within a single panel). First, for the particular mode that was cooled down by a single frequency ($k=N/4$), adding more frequencies has a detrimental effect. This can be understood since the frequencies that are not resonant tend to heat that mode, which cannot be compensated by the resonant frequency. Across most panels in this finite sweep, using multiple frequencies ($R=11$ or $R=21$) moves the plotted spectrum closer to the ground-state curve than the single-frequency case ($R=1$), although this comparison is mode dependent. For example, in panel (c), the multi-frequency lines (dashed and dash-dotted) are significantly closer to the ground state across the entire spectrum compared to the single-frequency line (solid blue).

However, the advantage of using multiple frequencies decreases as the coupling strength $g$ or the noise strength $\kappa$ increases. For strong coupling ($g=0.1$, bottom row), the difference between $R=1$, $R=11$, and $R=21$ is less pronounced, especially in the presence of noise (panels (h), (i)). Thus \cref{fig:FigNew3} should be read as a finite parameter sweep showing partial mitigation of reheating, not as a parameter-uniform robustness statement. Even for the relatively large coupling $g=0.1$ (bottom row), multi-frequency cooling still offers some improvement over single-frequency cooling in parts of the sweep, particularly in the noiseless case (panel (g)). This motivates the multi-frequency approach combined with randomized times, while leaving the quantitative noise tolerance to the conditions below.
...
\textit{Note added.}---While finalizing this manuscript, we became aware of a related work by Zhan et al.~\cite{Zhan2026Rapid} on ground state preparation via dissipative dynamics.
