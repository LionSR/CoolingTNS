...
@testset "Mode Analysis" begin
    ...
    @testset "Bogoliubov text matches MapToSpin phase convention" begin
        ...
        @test file_contains(cooling_tn_path, raw"\texttt{eq:bog\_occupation\_from\_hk}")
        @test file_contains(cooling_tn_path, raw"\texttt{eq:raw\_fourier\_occupation}")
        @test file_contains(cooling_tn_path, raw"rather than an unqualified \(n_k\)")
