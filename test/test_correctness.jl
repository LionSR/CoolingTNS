...
@testset "ED Interleaving Correctness" begin
    ...
    # `prepare_combined_state_ed` is the production bath-append entry point that
    # `prepare_combined_state` uses on the ED backend. Pin its coupling default,
    # its normalization, and the agreement of its two representations.
    ψ_combined_xx = CoolingTNS.prepare_combined_state_ed(ψ_sys, N)
    ψ_combined_xx_explicit = CoolingTNS.prepare_combined_state_ed(ψ_sys, N, "XX")
    @test norm(ψ_combined_xx.data - ψ_combined_xx_explicit.data) < 1e-12
    @test abs(norm(ψ_combined_xx.data) - 1.0) < 1e-12

    ψ_combined_zz = CoolingTNS.prepare_combined_state_ed(ψ_sys, N, "ZZ")
    @test norm(ψ_combined_zz.data - ψ_combined_xx.data) > 1e-3

    ρ_sys = CoolingTNS.state_to_density_ed(ψ_sys)
    ρ_combined_xx = CoolingTNS.prepare_combined_state_ed(ρ_sys, N)
    ρ_combined_xx_explicit = CoolingTNS.prepare_combined_state_ed(ρ_sys, N, "XX")
    @test norm(ρ_combined_xx.data - ρ_combined_xx_explicit.data) < 1e-12

    # The density-matrix path must reproduce the density of the state-vector path.
    @test norm(ρ_combined_xx.data - CoolingTNS.state_to_density_ed(ψ_combined_xx).data) < 1e-12

    ρ_combined_zz = CoolingTNS.prepare_combined_state_ed(ρ_sys, N, "ZZ")
    @test norm(ρ_combined_zz.data - CoolingTNS.state_to_density_ed(ψ_combined_zz).data) < 1e-12
...
@testset "Partial Trace Consistency" begin
    ...
    # `trace_out_system_ed` is the production bath-extraction path taken by
    # `process_bath_and_update` on the ED density-matrix backend.
    ...
    ρ_bath_expected = CoolingTNS.state_to_density_ed(ψ_bath_layout).data
    ρ_bath_extracted = CoolingTNS.trace_out_system_ed(ρ_layout, N).data
    ...
    @test tr(ρ_bath_extracted) ≈ 1.0 atol=1e-12
    ...
    ρ_bath_coherent = CoolingTNS.trace_out_system_ed(ρ_coherent, N).data
    ...
    # `trace_out_bath_ed` is the production system-reduction path; it must keep
    # complex coherences intact in the interleaved layout.
    ...
    ρ_complex_reduced = CoolingTNS.trace_out_bath_ed(ρ_complex_combined, 1).data

    @test ρ_complex_reduced ≈ CoolingTNS.state_to_density_ed(ψ_complex).data atol=1e-12
    @test abs(imag(ρ_complex_reduced[1, 2])) > 1e-12
    @test imag(ρ_complex_reduced[1, 2]) ≈ -0.5 atol=1e-12

    ρ_real_combined = CoolingTNS.EDDensityMatrix(Matrix{ComplexF64}(I, 4, 4) / 4, 2)
    ρ_real_reduced = CoolingTNS.trace_out_bath_ed(ρ_real_combined, 1).data
