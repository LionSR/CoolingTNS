...
@testset "Bogoliubov Mode Observables" begin
    ...
    @testset "Momentum grid helper fallback and cache source" begin
        ...
        sim_params = UnifiedSimulationParameters(DensityMatrix(), ContinuousEvolution())
        problem = setup_problem(EDBackend(), ham_params, coupling_params, sim_params)
        mixed_state = QuantumState(EDBackend(), DensityMatrix(), ContinuousEvolution(), ρ_mix)
        mode_measurements = Dict{String, Any}()
        @test CoolingTNS._add_ising_mode_measurement_slots!(
            mode_measurements,
            problem,
            mixed_state,
            ham_params,
    ...
    @testset "Mode occupations are exposed with h_k results" begin
        ...
        bad_nan_nk = copy(mode_nk)
        bad_nan_nk[2, 1] = NaN
        @test_throws ArgumentError validate_mode_nk_matches_hk(bad_nan_nk, mode_hk)
        bad_nan_hk = copy(mode_hk)
        bad_nan_hk[2, 1] = NaN
        @test_throws ArgumentError validate_mode_nk_matches_hk(mode_nk, bad_nan_hk)
