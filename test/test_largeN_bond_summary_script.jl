...
function write_minimal_mode_summary_file(path::AbstractString, mode_hk, mode_nk;
    ...
    h5open(path, "w") do f
        ...
        write(gr, LARGE_N_SYSTEM_MEAN_BOND_KEY, ones(Float64, n_rows))
        write(gr, LARGE_N_EVOLVED_MAX_BOND_KEY, n_rows == 1 ? [0] : vcat(0, ones(Int, n_rows - 1)))
        write(gr, LARGE_N_EVOLVED_MEAN_BOND_KEY, n_rows == 1 ? [NaN] : vcat(NaN, ones(Float64, n_rows - 1)))
...
function write_split_trajectory_summary_file(
    ...
    h5open(path, "w") do f
        ...
        write(gr, CoolingTNS.RESULT_GROUND_STATE_OVERLAP_TRAJECTORIES,
              reshape(overlap, :, 1))
        write(gr, LARGE_N_SYSTEM_MAX_BOND_KEY, Int.(system_max))
        write(gr, LARGE_N_SYSTEM_MEAN_BOND_KEY, Float64.(system_max))
        write(gr, LARGE_N_EVOLVED_MAX_BOND_KEY, Int.(evolved_max))
        write(gr, LARGE_N_EVOLVED_MEAN_BOND_KEY, Float64.(evolved_max))
...
@testset "Large-N summary rejects unknown explicit schedule metadata" begin
    for schedule_level in (:root, :method, :run)
        path = tempname() * ".h5"
        try
            h5open(path, "w") do f
                write(f, LARGE_N_ROOT_DMAX_KEY, 8)
                schedule_level === :root &&
                    write(f, CoolingTNS.RESULT_SCHEDULE, "reverse")
                gn = create_group(f, largeN_n_group_name(4))
                write(gn, LARGE_N_SYSTEM_SIZE_KEY, 4)
                gm = create_group(gn, "mcwf")
                schedule_level === :method &&
                    write(gm, CoolingTNS.RESULT_SCHEDULE, "reverse")
                gr = create_group(gm, largeN_r_group_name(2))
                schedule_level === :run &&
                    write(gr, CoolingTNS.RESULT_SCHEDULE, "reverse")
    ...
    for schedule_level in (:root, :method, :run)
        ...
        try
            h5open(path, "w") do f
                ...
                gm = create_group(gn, "mcwf")
                schedule_level === :method &&
                    write(gm, CoolingTNS.RESULT_SCHEDULE, 7)
                gr = create_group(gm, largeN_r_group_name(2))
                schedule_level === :run &&
                    write(gr, CoolingTNS.RESULT_SCHEDULE, 7)
...
@testset "Large-N bond-dimension summary script" begin
    ...
    try
        h5open(path, "w") do f
            ...
            write(gr, LARGE_N_SYSTEM_MAX_BOND_KEY, [1 1; 6 9; 12 10])
            write(gr, LARGE_N_SYSTEM_MEAN_BOND_KEY, [1.0 1.0; 4.0 5.0; 8.0 5.0])
            write(gr, LARGE_N_EVOLVED_MAX_BOND_KEY, [0 0; 12 6; 8 14])
            write(gr, LARGE_N_EVOLVED_MEAN_BOND_KEY, [NaN NaN; 8.0 7.0; 9.0 11.0])
...
@testset "Large-N summary combines split trajectory-axis files" begin
    ...
    try
        ...
        split_by_g_rows = combine_trajectory_rows(vcat(
            summarize_file(path1),
            summarize_file(g_mismatch_path),
        ))
        @test length(split_by_g_rows) == 2
        @test sort([row.g for row in split_by_g_rows]) == [0.05, 0.3]
...
@testset "Large-N summary validates trajectory metadata fallbacks" begin
    function write_legacy_split_metadata_file(path; trajectory_indices=nothing,
        ...
        h5open(path, "w") do f
            ...
            write(gr, LARGE_N_SYSTEM_MEAN_BOND_KEY, [1.0 1.0; 2.0 3.0])
            write(gr, LARGE_N_EVOLVED_MAX_BOND_KEY, [0 0; 3 4])
            write(gr, LARGE_N_EVOLVED_MEAN_BOND_KEY, [NaN NaN; 3.0 4.0])
...
@testset "Large-N summary handles missing detuning metadata" begin
    ...
    try
        h5open(path, "w") do f
            ...
            write(gr, LARGE_N_EVOLVED_MAX_BOND_KEY, [0, 4])
            write(gr, LARGE_N_EVOLVED_MEAN_BOND_KEY, [NaN, 4.0])
        end
...
@testset "Large-N summary distinguishes empty truncation-error histories" begin
    ...
    try
        h5open(path, "w") do f
            ...
            write(gr_empty, LARGE_N_SYSTEM_MEAN_BOND_KEY, [1.0, 2.0])
            write(gr_empty, LARGE_N_EVOLVED_MAX_BOND_KEY, [0, 3])
            ...
            write(gr_explicit, LARGE_N_SYSTEM_MEAN_BOND_KEY, [1.0, 2.0])
            write(gr_explicit, LARGE_N_EVOLVED_MAX_BOND_KEY, [0, 3])
            write(gr_explicit, LARGE_N_EVOLVED_MEAN_BOND_KEY, [NaN, 3.0])
...
@testset "Large-N summary reports mode energy reconstruction" begin
    ...
    try
        ...
        h5open(path, "w") do f
            ...
            write(gr, LARGE_N_SYSTEM_MAX_BOND_KEY, [1, 2, 3])
            write(gr, LARGE_N_SYSTEM_MEAN_BOND_KEY, [1.0, 2.0, 3.0])
            write(gr, LARGE_N_EVOLVED_MAX_BOND_KEY, [0, 3, 4])
            write(gr, LARGE_N_EVOLVED_MEAN_BOND_KEY, [NaN, 3.0, 4.0])
            write(gr, CoolingTNS.RESULT_MODE_HK, mode_hk)
...
@testset "Large-N summary rejects partial mode metadata" begin
    ...
    try
        h5open(path, "w") do f
            ...
            write(gr, LARGE_N_EVOLVED_MAX_BOND_KEY, [0])
            write(gr, LARGE_N_EVOLVED_MEAN_BOND_KEY, [NaN])
