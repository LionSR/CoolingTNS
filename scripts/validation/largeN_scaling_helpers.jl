...
_invalid_largeN_group_name_error(label::AbstractString, prefix::AbstractString,
                                 name::AbstractString) = ArgumentError(
    "large-N HDF5 $label-group name must have canonical form " *
    "$prefix<positive integer> without leading zeros, " *
    "got $(repr(String(name)))"
)

function _largeN_index_from_group_name(name::AbstractString, prefix::AbstractString,
                                       label::AbstractString)
    startswith(name, prefix) ||
        throw(_invalid_largeN_group_name_error(label, prefix, name))
    suffix = name[(lastindex(prefix) + 1):end]
    occursin(r"^[1-9][0-9]*$", suffix) ||
        throw(_invalid_largeN_group_name_error(label, prefix, name))
    index = tryparse(Int, suffix)
    index === nothing && throw(ArgumentError(
        "large-N HDF5 $label-group name is too large for Int: $(repr(String(name)))"
    ))
    return index
end
largeN_n_from_group_name(name::AbstractString) =
    _largeN_index_from_group_name(name, LARGE_N_N_GROUP_PREFIX, "N")
largeN_r_from_group_name(name::AbstractString) =
    _largeN_index_from_group_name(name, LARGE_N_R_GROUP_PREFIX, "R")
...
const LARGE_N_PROGRESS_STOP_ON_BOND_CAP_KEY = LARGE_N_ROOT_STOP_ON_BOND_CAP_KEY
