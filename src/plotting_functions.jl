...
function plotOptimal_energy_error_and_overlap_vs_N_pe_range(ham_name, coupling_params, sim_params, search_params, N_values, peInt_range)
    plt = pyimport("matplotlib.pyplot")
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 4))
    ...
    for peInt in peInt_range
        ...
        for N in N_values
            ham_name_part = "Ham$(ham_name)Ns$(N)Nb$(N)"
            filename = "OptimizedCooling_Problem$(ham_name_part)_Paramssteps$(coupling_params["steps"])_$(sim_name_part)_$(search_name_part)_peInt$(peInt)"
